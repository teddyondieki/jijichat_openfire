...
/**
 * A StanzaHandler is the main responsible for handling incoming stanzas. Some
 * stanzas like startTLS are totally managed by this class. The rest of the
 * stanzas are just forwarded to the router.
 *
...
public abstract class StanzaHandler {
     ...
     * {@link org.jivesoftware.openfire.interceptor.PacketInterceptor} will be
     * invoked before and after the packet was routed.
     * <p>
     * Subclasses may redefine this method for different reasons such as
     * modifying the sender of the packet to avoid spoofing, rejecting the
     * packet or even process the packet in another thread.</p>
     *
     * @param packet the received packet.
     * @throws org.jivesoftware.openfire.auth.UnauthorizedException if service
     * is not available to sender.
     */
    ...
    /**
     * Returns true if a received packet of an unkown type (i.e. not a Message,
     * Presence or IQ) has been processed. If the packet was not processed then
     * an exception will be thrown which will make the thread to stop processing
     * further packets.
     *
     * @param doc the DOM element of an unkown type.
     * @return true if a received packet has been processed.
     * @throws UnauthorizedException if stanza failed to be processed.
     * Connection will be closed.
     */
    abstract boolean processUnknowPacket(Element doc) throws UnauthorizedException;

    /**
     * Tries to secure the connection using TLS. If the connection is secured
     * then reset the parser to use the new secured reader. But if the
     * connection failed to be secured then send a <failure> stanza and close
     * the connection.
     *
    ...
    /**
     * TLS negotiation was successful so open a new stream and offer the new
     * stream features. The new stream features will include available SASL
     * mechanisms and specific features depending on the session type such as
     * auth for Non-SASL authentication and register for in-band registration.
     */
    ...
    /**
     * After SASL authentication was successful we should open a new stream and
     * offer new stream features such as resource binding and session
     * establishment. Notice that resource binding and session establishment
     * should only be offered to clients (i.e. not to servers or external
     * components)
     */
    ...
    /**
     * Start using compression but first check if the connection can and should
     * use compression. The connection will be closed if the requested method is
     * not supported, if the connection is already using compression or if
     * client requested to use compression but this feature is disabled.
     *
     * @param doc the element sent by the client requesting compression.
     ...
     * new stream features such as resource binding and session establishment.
     * Notice that resource binding and session establishment should only be
     * offered to clients (i.e. not to servers or external components)
     */
    ...
    /**
     * Close the connection since TLS was mandatory and the entity never
     * negotiated TLS. Before closing the connection a stream error will be sent
     * to the entity.
     */
     ...
     * either creates a session or returns an error and kills the connection. If
     * the connection remains open, the XPP will be set to be ready for the
    ...
    protected void createSession(XmlPullParser xpp) throws XmlPullParserException, IOException {
        ...
        if (validateHost() && isHostUnknown(host)) {
            ...
            Log.warn("Closing session due to incorrect hostname in stream header. Host: " + host
                    + ". Connection: " + connection);
        } else if (!validateJijiChatClient(xpp)) {
            // Include the unsupported version in the response
            streamError = new StreamError(StreamError.Condition.unsupported_version);
            // Log a warning so that admins can track this cases from the server side
            Log.warn("Closing session due to unsupported version in stream header. Connection: " + connection);
    ...
    /**
     * Returns true if the value of the 'to' attribute in the stream header
     * should be validated. If the value of the 'to' attribute is not valid then
     * a host-unknown error will be returned and the underlying connection will
     * be closed.
     *
     * @return true if the value of the 'to' attribute in the initial stream
     * header should be validated.
     */
    abstract boolean validateHost();

    /**
     * Returns true if the value of the 'to' attribute of
     * {@link IQ}, {@link Presence} and {@link Message} must be validated.
     * Connection Managers perform their own JID validation so there is no need
     * to validate JIDs again but when clients are directly connected to the
     * server then we need to validate JIDs.
     *
     * @return rue if the value of the 'to' attribute of IQ, Presence and
     * Messagemust be validated.
     */
    abstract boolean validateJIDs();

    /**
     * Creates the appropriate {@link Session} subclass based on the specified
     * namespace.
     *
     * @param namespace the namespace sent in the stream element. eg.
     * jabber:client.
...
// Added
        try {
            ...
            String data = md5(from + clientVersion);

            if ((clientVersion < JiveGlobals.getIntProperty("jiji.minClientVer", 1)) || (!data.equals(key))) {
                return false;
            }
        } catch (Exception ex) {
            return false;
...
// End of added
    ...
    private static String md5(String data) {
        try {
            ...
            throw new RuntimeException("Error on creating key", e);
        }
    }
